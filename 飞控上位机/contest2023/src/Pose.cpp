...
void raverseMapBySpeed_Block(ros::Rate &rate)
{
    ...
    while (true)
    {
        ...
        {
        case 1: // 前进至（1.2，0）处
            if (controlXSpeed_YLimit(1.2, start_x, start_y))
            {
                ROS_WARN("Arrive (1.2,0)");
                start_x = lidar_pose.x;
                start_y = lidar_pose.y;
                mode = 2;
            }
            break;
        case 2: // 前进至（1.2，-3.8）处
            if (controlYSpeed_XLimit(-3.8, start_x, start_y))
            {
                ROS_WARN("Arrive (1.2,-3.8)");
                start_x = lidar_pose.x;
                start_y = lidar_pose.y;
                mode = 3;
            }
            break;
        case 3: // 前进至（0.6，-3.8）处
            if (controlXSpeed_YLimit(0.6, start_x, start_y))
            {
                ROS_WARN("Arrive (0.6,-3.8)");
                start_x = lidar_pose.x;
                start_y = lidar_pose.y;
                mode = 4;
            }
            break;
        case 4: // 前进至（2.7，-3.8）处
            if (controlXSpeed_YLimit(2.7, start_x, start_y))
            {
                ROS_WARN("Arrive (2.7,-3.8)");
                start_x = lidar_pose.x;
                start_y = lidar_pose.y;
                mode = 5;
            }
...
bool fireProcess_Block(ros::Rate &rate)
{
    ...
    // 降落至1m左右
    double heightChange = lidar_pose.z - 1.0;
